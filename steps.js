...
const StepsScreen = () => {
    ...
    // 🔥🔥 التحديث اللحظي للخطوات (دون كراش) 🔥🔥
    useEffect(() => {
        let observer = null;
        if (isGoogleFitConnected) {
            observer = GoogleFit.observeSteps((res) => {
                // نجمع الخطوات الجديدة التي وصلت في هذا الحدث
                if (res && res.steps && res.steps.length > 0) {
                    let newStepsToAdd = 0;
                    res.steps.forEach(s => {
                        newStepsToAdd += s.value || 0;
                    });
                    // تحديث الواجهة فقط (بدون طلب بيانات من قاعدة البيانات)
                    if (newStepsToAdd > 0) {
...
export default StepsScreen;import React, { useState, useEffect, useCallback, useLayoutEffect, useRef } from 'react';
import { 
    StyleSheet, View, Text, ScrollView, SafeAreaView, TouchableOpacity, 
    ActivityIndicator, Alert, Modal, TextInput, StatusBar,
    Platform, PermissionsAndroid, AppState, InteractionManager, DeviceEventEmitter 
